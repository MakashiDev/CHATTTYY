	...
	jwt.verify(token, secretKey, (err, decoded) => {
		if (err) {
			...
			db.run(
				...
				[groupname, user_id, timestamp],
				(err) => {
					if (err) {
						res.send({ error: err.message });
					} else {
						db.get(
							"SELECT * FROM groups WHERE group_name = ? AND created_by = ? AND created_at = ?",
							[groupname, user_id, timestamp],
							(err, row) => {
								if (err) {
									res.send({ error: err.message });
											...
											(err, row) => {
												if (err) {
													...
													if (row) {
														...
														db.run(
															"INSERT INTO group_members (group_id, user_id, username, joined_at) VALUES (?, ?, ?, ?)",
															[
																group_id,
																members[i]
																	.user_id,
				...
				(err, row) => {
					if (err) {
						...
						if (row) {
							...
							res.status(401).send({
								error: "You are not authorized to change this group name",
							});
						}
					}
				}
